...
def setup_training_options(
    ...
    # ! set intermediate label as average, intermediate has to be last in alphabet ordering
    intermediate_state = False,

    # ! split label emb into 2 sets, diseases + age
    split_label_emb_at = 0, 

    # ! mix/oversample a label
    oversample_prob     = 0,
    oversample_index    = None,
    mix_labels          = 0
    ...
    args.oversample_prob = oversample_prob   
    args.oversample_index = oversample_index
    args.mix_labels = mix_labels        
    ...
    # ! use average of early+late = intermediate 
    if intermediate_state: 
        desc += f'-setmid'
        args.G_args.intermediate_state = True
        args.D_args.intermediate_state = True

    # ! split label emb into 2 sets (smaller number params)
    if split_label_emb_at: 
        desc += f'-divlabel'+str(split_label_emb_at)
        args.G_args.split_label_emb_at = split_label_emb_at
        args.D_args.split_label_emb_at = split_label_emb_at
        
    # ! add to @desc
    if mix_labels > 0: 
        args.mix_labels = mix_labels
        desc += f'-mix{mix_labels:g}' 
...
def _parse_comma_sep_int(s):
    if s is None or s.lower() == 'none' or s == '':
        return None
    return [int(_s.strip()) for _s in s.split(',')]
...
def main():
    ...
    # ! 
    group.add_argument('--intermediate_state', help='average early+late=intermediate', action='store_true', default=False)

    # ! 
    group.add_argument('--split_label_emb_at', help='divide label into 2 emb at this number.', type=int, default=0)

    # ! mix/oversample a label
    group.add_argument('--oversample_prob',      help='oversample a label', type=float, metavar='FLOAT')
    group.add_argument('--oversample_index',     help='oversample this label index', type=_parse_comma_sep_int, metavar='LIST')
    group.add_argument('--mix_labels',           help='mix labels', type=float, default=0, metavar='FLOAT')
