...
def setup_snapshot_image_grid(training_set):
    ...
    for _iter in range(100000): #! make shorter than 1000000
    ...
    labels = np.zeros([gw * gh, training_set.label_size], dtype=training_set.label_dtype) # ! init with all zero, but why @labels have 0 if we replace it with our label?
    for i, block in enumerate(blocks):
        for j, (real, label) in enumerate(block):
            x = (i %  nw) * cw + j %  cw # ! position (0,0) is not used, so it's all blank. same as first row, all blank by "design" of this code
            y = (i // nw) * ch + j // cw
            if x < gw and y < gh:
                reals[x + y * gw] = real[0]
...
def training_loop(
    ...
    print('see training_set.shape[1] {}'.format(training_set.shape[1])) # ! check resolution
    ...
    # ! save @grid_latents once so we can recreate the images. ?
    # ! example call: images = Gs.run(z, label, **Gs_kwargs) # [minibatch, height, width, channel]
    ...
    for gpu, (G_gpu, D_gpu) in enumerate(zip(G_gpus, D_gpus)):
        with tf.name_scope(f'Train_gpu{gpu}'), tf.device(f'/gpu:{gpu}'):
            ...
            if oversample_prob == 0: 
                fake_labels = training_set.get_random_labels_tf(minibatch_gpu) # ! sample a label for fake images
            else: 
                fake_labels = training_set.get_random_labels_np(minibatch_gpu, oversample_prob, oversample_index, mix_labels) # ! sample a label for fake images in np, so we can mess with labels
