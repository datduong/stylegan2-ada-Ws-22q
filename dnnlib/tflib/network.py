    ...
    def copy_own_vars_from(self, src_net: "Network") -> None:
        ...
        if self._var_inits is None:
            try: 
                ...
                print ('fails to init some variables dude to size mismatch, happens if we load pretrained facedata and then use for our own purpose')
                # for name, value in value_dict.items(): 
                #     print (name)
                skip = ['Output/weight','Output/bias']
                print ('Fail @{} so we skip ...'.format(skip))
                tfutil.set_vars({self._get_vars()[name]: value for name, value in value_dict.items() if name in self._get_vars() and name not in skip})
        else:
        ...
        for name, src_comp in src_net._components.items():
            if name in self._components:
                try: 
                    ...
                    print ('Fail @{} so we skip ...'.format(name))
                    pass # ! just skip ??
