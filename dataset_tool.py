...
def create_from_images(tfrecord_dir, image_dir, shuffle, resolution=None, classifier_train_csv=None, fold=None, selectlabel=None):
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    if len(image_filenames) == 0:
        error('No input images found')

    if classifier_train_csv is not None: 
        # remove the fold. 
        classifier_train_csv = pd.read_csv ( classifier_train_csv ) 
        classifier_train_csv = classifier_train_csv[classifier_train_csv['fold'] != fold ] 
        classifier_train_csv = classifier_train_csv[classifier_train_csv['label'] == selectlabel ] # ! keep labels needed
        print ('num ob before remove fold {} , keep label {} , {}'.format(fold, selectlabel, len(image_filenames)))
        # image_filenames is whole path
        image_filenames = list ( classifier_train_csv.path.values )
        print ('num ob after remove fold {} , {}'.format(fold, len(image_filenames)))
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            img = PIL.Image.open(image_filenames[order[idx]]).convert('RGB')
...
def create_from_images_with_labels(tfrecord_dir, image_dir, shuffle, label_names, resolution=None):
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    if len(image_filenames) == 0:
        error('No input images found')

    # ! mixing gray scale with color.... not ideal, we will just replicate values of grayscale 
    
    print ('take this img to define resolution and grayscale{}'.format(image_filenames[0]))
    img = np.asarray( PIL.Image.open(image_filenames[0]).convert('RGB') ) # ! open first image to check, notice the ".convert('RGB')"
    if resolution is None:
        resolution = img.shape[0]
        if img.shape[1] != resolution:
            error('Input images must have the same width and height')
    ...
    # ! added code
    label_names = sorted ( label_names.split(',') )
    assert len(label_names) > 0
    label_names_dict = {v:i for i,v in enumerate(label_names)} # ! just a dictionary
    num_labels = len(label_names_dict)
    print ('number of labels {}'.format(num_labels))
    num_obs = len(image_filenames)
    onehot = np.zeros((num_obs, num_labels), dtype=np.float32)
    image_filenames = sorted(image_filenames) # ! just to make life easy.
    labels = np.zeros(num_obs, dtype=np.int32)
    for idx,filename in enumerate(image_filenames): 
        for k,v in label_names_dict.items(): # ! iterate over labels, assign index names
            if k in filename.split('/')[-1]: # ! because of our convention, we can create label based on image_filenames[order[idx]]
                labels[idx] = v # take this label 
    onehot[np.arange(num_obs), labels] = 1.0 # ! onehot labels, num_obs x num_labels
    print ('label indexing {}'.format(label_names_dict))
    print ('onehot labels shape {}'.format(onehot.shape))
    assert onehot.shape[0] == len(labels)
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        # ! because of our convention, we can create label based on image_filenames[order[idx]]
        tfr.add_labels(onehot[order])


def create_from_images_with_labels_fromjson (tfrecord_dir, image_dir, shuffle, labeljson_name='dataset.json', resolution=None, oversample_label=None, oversample_time=None, classifier_train_csv=None, fold=None, use_these_label=None):

    # ! can change this to take in an array of folder path 
    image_dir_array = image_dir.split(',')
    image_filenames = []
    for _dir in image_dir_array: 
        print('Loading images from "%s"' % _dir)
        image_filenames = image_filenames + [j for j in sorted(glob.glob(os.path.join(_dir.strip(), '*'))) if 'json' not in j]
    #
    ...
    img = np.asarray(PIL.Image.open(image_filenames[0]).convert('RGB')) # ! open first image to check
    if resolution is None:
        resolution = img.shape[0]
        if img.shape[1] != resolution:
            error('Input images must have the same width and height')
    ...
    labeljson = []
    for _dir in image_dir_array: 
        with open(os.path.join(_dir,labeljson_name), 'rb') as f:
            labeljson = labeljson + json.load(f)['labels']

    # ! @labeljson into 
    # https://stackoverflow.com/questions/13675296/how-to-overcome-typeerror-unhashable-type-list
    # labeljson = set (labeljson) # should be unique already # ! @labeljson is a list of list, so can't use set. 
    # check 
    for l in labeljson: 
        if len(l) != 2: 
            sys.exit('check json, each entry should have len=2')
    #
    labeljson = {i[0]:i[1] for i in labeljson }
    
    num_labels = len(labeljson[image_filenames[0].split('/')[-1]]) # take first obs, see how many labels
    print ('number of labels {}'.format(num_labels))

    # ! remove "fold" that we have in the test set, USE ONLY IMAGES IN TRAIN SET FOR CLASSIFIER
    if classifier_train_csv is not None: 
        # remove the fold. 
        classifier_train_csv = pd.read_csv ( classifier_train_csv ) 
        if fold is not None: 
            print ('num ob before remove fold {} , {}'.format(fold, classifier_train_csv.shape[0]))
            classifier_train_csv = classifier_train_csv[classifier_train_csv['fold'] != fold ]
            print ('num ob after remove fold {} , {}'.format(fold, classifier_train_csv.shape[0]))
        # image_filenames is whole path
        if use_these_label is not None: 
            print ('keep just these labels {}'.format(use_these_label)) 
            classifier_train_csv = classifier_train_csv[classifier_train_csv.label.isin(use_these_label.split(','))]
        #
        image_filenames = list ( classifier_train_csv.path.values ) # ! not sort to make backtrack easier
        print ('num ob to be used {}'.format(len(image_filenames)))
    
    # ! can oversample by replicating subset of @image_filenames
    if oversample_label is not None: # https://stackoverflow.com/questions/38974168/finding-entries-containing-a-substring-in-a-numpy-array
        print ('num ob before oversample {}'.format(len(image_filenames)))
        image_filenames = np.array (image_filenames)
        temp_ = []
        for lab in sorted(oversample_label.split(',')):
            location = np.flatnonzero(np.core.defchararray.find(image_filenames,lab)!=-1)
            temp_ = temp_ + image_filenames[location].tolist() # images that will later get duplicated 
        # 
        if (oversample_time is None) or (oversample_time==0): 
            oversample_time = 1
        temp_ = temp_ * oversample_time # duplicate 
        image_filenames = image_filenames.tolist() + temp_ 
        print ('new num ob {}'.format(len(image_filenames)))

    # ! labels were made from Fairface alignment, this alignment can fail for some images. 
    print ('check datajson has all images, and remove images not found in json, size {}'.format(len(image_filenames)))
    image_filenames = [filename for filename in image_filenames if filename.split('/')[-1] in labeljson]
    num_obs = len(image_filenames)
    print ('after check json size {}'.format(num_obs))

    onehot = np.zeros((num_obs, num_labels), dtype=np.float32) # ! may not be true 1hot
    for idx,filename in enumerate(image_filenames): 
        onehot[idx] = labeljson[filename.split('/')[-1]] # just name, not path        
                
    print ('labels shape {}'.format(onehot.shape)) # ! may not be true 1hot
    print (onehot)
    print (np.sum(onehot,axis=0))

    # ! original code
    # ! can change @image_filenames to oversample an observation 
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames))
        for idx in range(order.size):
            img = PIL.Image.open(image_filenames[order[idx]]).convert('RGB')
        ...
        tfr.add_labels(onehot[order]) # ! add in label
...
def execute_cmdline(argv):
    ...
    p.add_argument(     '--resolution',     help='Output resolution (default: 256)', type=int, default=256)
    p.add_argument(     '--classifier_train_csv',      help='classifier_train_csv', type=str, default=None)
    p.add_argument(     '--fold',      help='fold', type=int, default=None)
    p.add_argument(     '--selectlabel',      help='selectlabel', type=str, default=None)

    # ! our code
    p = add_command(    'create_from_images_with_labels', 'Create dataset from a directory full of images.',
                                            'create_from_images_with_labels datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
    p.add_argument(     '--label_names',    help='string of names cat,dog,...', type=str, default=None)
    p.add_argument(     '--resolution',     help='Output resolution (default: 256)', type=int, default=256)

    # ! our code
    p = add_command(    'create_from_images_with_labels_fromjson', 'Create dataset from a directory full of images.',
                                            'create_from_images_with_labels datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
    p.add_argument(     '--labeljson_name',      help='json', type=str, default='dataset.json')
    p.add_argument(     '--resolution',     help='Output resolution (default: 256)', type=int, default=256)
    p.add_argument(     '--oversample_label',      help='oversample_label', type=str, default=None)
    p.add_argument(     '--oversample_time',      help='oversample_time', type=int, default=None)
    p.add_argument(     '--classifier_train_csv',      help='classifier_train_csv', type=str, default=None)
    p.add_argument(     '--fold',      help='fold', type=int, default=None)
    p.add_argument(     '--use_these_label',      help='comma sep', type=str, default=None)
