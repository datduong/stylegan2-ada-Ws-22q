...
class TFRecordDataset:
    def __init__(self,
        ...
        self._np_labels = np.zeros([1<<30, 0], dtype=np.float32) # ! create blank space-holder
        if self.label_file is not None and max_label_size != 0:
            self._np_labels = np.load(self.label_file) # ! load in numpy file, size num_obs x num_labels
            assert self._np_labels.ndim == 2
            # ! check nothing is zero? 
            if len ( np.where(~self._np_labels.any(axis=1))[0] ) > 0 : 
                print ('\n\nsome obs have labels all 0, is this okay??\n\n')
        #
        ...
        print ('load in label_file {}'.format(self.label_file)) # ! should detect these by default
        print ('load in label_size {}'.format(self.label_size))
    ...
    def get_minibatch_tf(self): # => images, labels # ! labels is already incorporated. 
        images, labels = self._tf_iterator.get_next()
        if self.mirror_augment: # ! now we do aug on the image, so add @mirror_augment
            images = tf.cast(images, tf.float32)
            images = tf.where(tf.random_uniform([tf.shape(images)[0]]) < 0.5, images, tf.reverse(images, [3]))
            images = tf.cast(images, self.dtype)
        return images, labels # ! labels is here. 
    ...
    def get_random_labels_tf(self, minibatch_size): # => labels
        with tf.name_scope('Dataset'):
            if self.label_size > 0:
                # ! should we oversample our labels? 
                # ! it looks like they gather labels vectors, and put these vec into random orders
                # ! can't assign items to tensorflow array
                with tf.device('/cpu:0'):
                    return tf.gather(self._tf_labels_var, tf.random_uniform([minibatch_size], 0, self._np_labels.shape[0], dtype=tf.int32))
            return tf.zeros([minibatch_size, 0], self.label_dtype)

    # Get random labels as NumPy array.
    def get_random_labels_np(self, minibatch_size, oversample_prob=0, oversample_index=None, mix_labels=0): # => labels
        if self.label_size > 0:
            label = self._np_labels[np.random.randint(self._np_labels.shape[0], size=[minibatch_size])] # ! @label is 1-hot, so batch x label_dim
            # ! mix labels? 
            if mix_labels > np.random.uniform(): # no mix if mix_labels=0
                wt_ = np.random.uniform(size=(minibatch_size,1)) # 16x1 (so broadcoast)
                label_ = self._np_labels[np.random.randint(self._np_labels.shape[0], size=[minibatch_size])] # ! @label is 1-hot, so batch x label_dim
                label = label*wt_ + label_*(1-wt_)  
            # ! should we oversample our labels? 
            if oversample_prob>0: # we will over sample @oversample_index
                # len_ = len(oversample_index)
                row_ix = np.random.choice(minibatch_size, size=int(minibatch_size*oversample_prob), replace=False)
                # row_ix = row_ix.reshape((len_, row_ix.shape[0]//len_)) # ! may cause error ? if row_ix.shape[0]//len_ is not divisible? 
                up_label_ix_and_row_ix = list(zip(cycle(oversample_index), row_ix))
                for up_ in enumerate(up_label_ix_and_row_ix): 
                    label[up_[1],up_[0]] = 1 
            return label
        #
        else: 
