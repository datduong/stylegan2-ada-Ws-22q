...
def _create_run_dir_local(submit_config: SubmitConfig) -> str:
    ...
    if os.path.exists(run_dir):
        ...
        print ("The run dir already exists! {}".format(run_dir))
        now = datetime.now() # current date and time
        date_time = now.strftime("%m%d%Y%H%M%S")
        submit_config.run_name = submit_config.run_name+date_time
        run_dir = os.path.join(run_dir_root, submit_config.run_name) # ! avoid error when submit many jobs

    try: 
        os.makedirs(run_dir) # ! seems stupid, but if 2 jobs run at once, @if os.path.exists(run_dir): will not catch error.  
    except: 
        print ("The run dir already exists! {}".format(run_dir))
        now = datetime.now() # current date and time
        date_time = now.strftime("%m%d%Y%H%M%S")
