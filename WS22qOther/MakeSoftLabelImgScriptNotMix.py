...
python3 generate.py --outdir=$outdir --trunc=$truncationpsi --seeds=SEED --network $model --savew 0 --suffix 'F'$fold'C'$class'T'$truncationpsi$label --class=$class 
...
# make a joint label 
joint_label_index_gan = {}
for k1,v1 in labelset_head_in_gan.items(): 
  for k2,v2 in labelset_tail_in_gan.items(): 
    joint_label_index_gan[k1+k2] = str(v1) + ',' + str(v2)
...
for fold in [0,1,2,3,4]: 
  ...
  for label1 in labelset: 
    ...
    #
    class1 = joint_label_index_gan[label1]
    #
    classifier_train_csv = TRAINCSV[TRAINCSV['fold'] != fold ] 
    if UseAvergeDiseaseCount == False: 
      classifier_train_csv = classifier_train_csv[classifier_train_csv['label'] == label1 ] # ! look at WS2y, simply make more WS2y
      size = int ( classifier_train_csv.shape[0] ) # 400 # len(imglist)
    else: 
      # ! look at average count of WS, then use this number make WS2y, SO WE CAN MAKE MORE/LESS THAN THE ACTUAL WS2y
      size = int ( np.ceil( average_count_per_disease[label1] ) )
    #
