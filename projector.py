...
class Projector:
    def __init__(self, num_steps=100, classid=None, tiled=True):
        self.num_steps                  = num_steps
        self.dlatent_avg_samples        = 5000 # ! we don't need 10000, we have very small samples? 
        ...
        self.classid = classid # ! we have labels? should be batch x num_label in 1hot style
        self.tiled   = tiled # ! if true, then use W(1,*)
    ...
    def set_network(self, Gs, dtype='float16'):
        ...
        # ! we should replace None in the call to self._Gs.components.mapping.run
        if self.classid is None:
            labels = None
        else: 
            self._info ('Build label, take 1 class')
            labels = np.zeros( [self.dlatent_avg_samples] + Gs.input_shapes[1][1:] ) # just make label sample x 1 hot
            self.classid = np.fromstring(self.classid,sep=',',dtype=int) # pass in a vector
            if self.classid.sum() > 0 : # ! for fake images, we can use no label (labels are used to init W, W will be optim)
                labels[:, self.classid] = 1 # ! takes in label, but @class_idx is int, so we pass in just 1 label at a time.
        ...
        if self.tiled: 
            dlatent_samples = self._Gs.components.mapping.run(latent_samples, labels) # [N, L, C]
            dlatent_samples = dlatent_samples[:, :1, :].astype(np.float32) # [N, 1, C]
        else: # ! add tile
            dlatent_samples = self._Gs.components.mapping.run(latent_samples, labels).astype(np.float32) # [N, 18, 512]

        # dlatent_samples = self._Gs.components.mapping.run(latent_samples, labels)  # [N, L, C] # ! create W vectors, so we are not using Z.? is that possible?
        # dlatent_samples = dlatent_samples[:, :1, :].astype(np.float32)           # [N, 1, C]
        ...
        # ! looks like obs labels are not used to generate images. do we care? 
        ...
        # ! use tile or not ? tile->same W in all layers 
        if self.tiled: 
            # ! if true, then use W(1,*)
            self._dlatents_expr = tf.tile(self._dlatents_var + dlatents_noise, [1, self._Gs.components.synthesis.input_shape[1], 1]) # ! this is saved and used later, so interpolation of real images must be in W space
        else: 
    ...
    def start(self, target_images):
        ...
        dlatents = np.tile(self._dlatent_avg, [self._minibatch_size, 1, 1]) # ! replicate by batch, so we can optim by batch. 
...
def project(network_pkl: str, target_fname: str, outdir: str, save_video: bool, seed: int, tiled: bool):
    # Load networks.
    ...
    # w, h = target_pil.size
    # s = min(w, h) # ! used to crop images, but we don't crop
    ...
    with tqdm.trange(proj.num_steps) as t:
        for step in t:
            ...
            dist, loss = proj.step()
            t.set_postfix(dist=f'{dist[0]:.4f}', loss=f'{loss:.2f}') # not show ??
...
def main():
    ...
    parser.add_argument('--tiled',       help='False-->16x512', type=_str_to_bool, default=True)
