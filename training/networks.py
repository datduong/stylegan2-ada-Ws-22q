...
# ! added code ----------------------------------------------------------------------------
# Fully-connected layer. # ! but... we divide inputs into 2 parts
def dense_layer_split_label_emb(x, split_label_emb_at, fmaps, lrmul=1, weight_var='weight', trainable=True, use_spectral_norm=False):
    # ! same as @dense_layer ... but @get_weight take various inputs, so we apply diff. settings to @x_head, @x_tail
    if len(x.shape) > 2:
        x = tf.reshape(x, [-1, np.prod([d.value for d in x.shape[1:]])])
    # ! split head
    x_head = x[ :, 0:split_label_emb_at ]
    w_head = get_weight([x_head.shape[1].value, fmaps], lrmul=lrmul, weight_var='weight_head', trainable=trainable, use_spectral_norm=use_spectral_norm)
    w_head = tf.cast(w_head, x_head.dtype)
    x_head = tf.matmul(x_head, w_head)
    # ! split tail
    x_tail = x[ :, split_label_emb_at:: ]
    w_tail = get_weight([x_tail.shape[1].value, fmaps], lrmul=lrmul, weight_var='weight_tail', trainable=trainable, use_spectral_norm=use_spectral_norm)
    w_tail = tf.cast(w_tail, x_tail.dtype)
    x_tail = tf.matmul(x_tail, w_tail)
    # ! append 2 label sets into longer label emb
    y = tf.concat([x_head, x_tail], axis=1)    
    return y

# ! added code ----------------------------------------------------------------------------
# Fully-connected layer. # ! but... we average 2 label vectors

def dense_layer_with_average(x, fmaps, lrmul=1, weight_var='weight', trainable=True, use_spectral_norm=False):
    if len(x.shape) > 2:
        x = tf.reshape(x, [-1, np.prod([d.value for d in x.shape[1:]])])
    # ! @fmaps is 512, or some dim for label vector. 
    # ! x.shape[1].value - 1, because 3rd label vec is intermediate state
    w = get_weight([x.shape[1].value - 1, fmaps], lrmul=lrmul, weight_var=weight_var, trainable=trainable, use_spectral_norm=use_spectral_norm)
    w = tf.cast(w, x.dtype)
    w_average = tf.reduce_mean(w, 0) # ! take average of before+after
    w_average = tf.reshape(w_average, [1,fmaps]) # ! reshape [512] into [1,512]
    w_concat = tf.concat([w, w_average], axis=0) # ! append average which is "inter" condition as last label
    return tf.matmul(x, w_concat)
...
def G_main(
    ...
    **kwargs,                                       # Arguments for sub-networks (mapping and synthesis).
    ...
    # Update moving average of W. # ! W is @dlatent_avg
...
def G_mapping(
    ...
    intermediate_state      = False,         # ! use @dense_layer_with_average

    split_label_emb_at      = 0,            # ! split label emb into diseases + age, concat them 
    ...
    **_kwargs,                              # Ignore unrecognized keyword args.
    ...
    if label_size > 0:
        with tf.variable_scope('LabelEmbed'):
            ...
            y = labels_in # ! @labels_in is 1-hot batchsize x label 
            if intermediate_state: # ! added code
                y = apply_bias_act(dense_layer_with_average(y, fmaps=fmaps)) 
            elif split_label_emb_at: 
                y = apply_bias_act(dense_layer_split_label_emb(y, split_label_emb_at, fmaps=fmaps//2))
            else:   
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    if label_size > 0:
        ...
        with tf.variable_scope('LabelEmbed'):
            ...
            if intermediate_state: # ! added code
                y = apply_bias_act(dense_layer_with_average(y, fmaps=mapping_fmaps)) 
            elif split_label_emb_at: 
                y = apply_bias_act(dense_layer_split_label_emb(y, split_label_emb_at, fmaps=mapping_fmaps//2))
            else: 
                y = apply_bias_act(dense_layer(y, fmaps=mapping_fmaps)) 
            y = normalize_2nd_moment(y)
